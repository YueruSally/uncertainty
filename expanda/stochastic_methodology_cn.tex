...
对弧 $a=(i,j,m)$，标称运输时间记为 $\bar T_a$。当存在来源明确且单位为小时的
班次运行时间字段（例如 \texttt{TravelTime\_h}）时，优先使用班次运行时间；否则使用
\begin{equation}
  \bar T_a=\frac{d_a}{v_m}.
\end{equation}
含义或单位不明确的 \texttt{time} 字段不直接作为逐弧运行时间。
...
CAREC CPMM 数据按国家、口岸、运输方式和方向报告年度平均处理时间，因此分别建模
出境侧和入境侧：
\begin{equation}\label{eq:border_two_sides_cn}
  \tilde D_b^{(s)}=
  \tilde D_{u,\mathrm{out}}^{m,(s)}+
  \tilde D_{v,\mathrm{in}}^{m,(s)}.
\end{equation}
若数据表中的某个参数已经表示完整双边过境过程，则只能增加一次，避免重复计时。

每一侧的随机延误使用与式~\eqref{eq:travel_random_cn} 相同的截断并均值校正的
对数正态乘数。基线 CV 设为
\begin{equation}
...
\begin{equation}
 ...
 \tilde D_{u,\mathrm{out}}^{m,(s)},
 \tilde D_{v,\mathrm{in}}^{m,(s)}\ \forall b\right\}.
\end{equation}
同一场景、同一弧或口岸的实现值由所有批次和所有候选方案共享。场景集生成后冻结，
适应度评价函数内部不再调用随机数生成器。
...
基线迟到费率采用 Zhao 等使用的 50 USD/TEU/day，并统一换算为
$50/24=2.0833$ USD/TEU/hour \citep{Zhao2018}。当前原始工作簿中的
30--100 USD/TEU/hour 比该文献基准高约 14--48 倍，因此代码默认不使用这些值。只有在
